...
%         chart of the elements) paired with the number of atoms for each 
%         element <Cx>, i.e. ['E1 C1 E2 C2 E3 C3 etc.'].  
%         Spaces are not required, i.e. all the following cases are allowed, 
%         although the top case is the most readable.:
...
%               Set maxstates = realmax; if one wishes to keep *ALL* the 
%               states.  Note that this is practical only for small 
%               molecules, since for large molecules this will slow
%               to a catatonic crawl and run out of memory.
%--------------------------------------------------------------------------
% Output:  	Returns a two column matrix where the first column contains 
%           the exact masses (most probable ones) and the 
%           second column are the probabilities (pruned).
%--------------------------------------------------------------------------
