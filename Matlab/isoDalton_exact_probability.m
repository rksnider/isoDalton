...
%--------------------------------------------------------------------------
% Description:  isoDalton_exact_probability.m 
%               Returns the exact probability isotopic distribution
%               of a molecule.
%--------------------------------------------------------------------------
% isoDalton_exact_probability(molecule_string,maxstates,massdiff_threshold,minprob_threshold)
% Input:  molecule_string :
%         String of elements <Ex> (any of the first 112 on the periodic 
%         chart of the elements) paired with the number of atoms for 
%         each element <Cx>, i.e. ['E1 C1 E2 C2 E3 C3 etc.'].  
%         Spaces are not required, i.e. all the following cases are 
%         allowed, although the top case is the most readable.:
...
%                     set maxstates = realmax; if one wishes to keep *ALL* 
%                     the states.  Note that this is practical only for 
%                     small molecules, since for large molecules this will 
%                     slow to a crawl and run out of memory.
%
%         massdiff_threshold : mass difference threshold
%                     If the number of required states is greater than 
%                     maxstates, then all states with a mass difference 
%                     less than this threshold are merged.
%
%         minprob_threshold : minimum probability threshold
%                     If the number of required states is greater than 
%                     maxstates, and the mass difference threshold case 
%                     is not satisfied, then neighboring states with 
%                     probabilities less than this threshold are merged.
%--------------------------------------------------------------------------
% Output:  	Returns a two column matrix where the first column contains 
%           the masses and the second column are the probabilities 
%           (merged, i.e. added).
%--------------------------------------------------------------------------
